...
- Fix: Auto-load default_directory if last_directory empty or invalid (startup fallback)
- Fix: Play/pause button reliability (removed Qt checkable state, use backend state for toggle, allow pause when playing regardless of load status)
- Fix: Play button stays enabled when media stopped (only disable when no song selected)
- Fix: MP3 seeking accuracy in vocals mode (convert to CBR 320kbps, remove VBR seek table issues)
- Fix: Gap detection status updates in UI (emit songs.updated signal after status change)
- Fix: VLC seeks on first click (pause before seek to prime media state)
- Fix: Playhead position restored after waveform reload on mode switch
- Fix: Position label shows 00:00:00.000 on song load instead of blank
- Fix: Status filter dropdown debounced to prevent signal loops during programmatic updates
- Fix: Duplicate songs when rescanning directories (updates existing instead of adding duplicates)
- Fix: Cache path normalization prevents duplicates from mixed path separators
- Fix: Config file creation error on startup handled gracefully
- Fix: Dropdown stays open when clicking text box (proper event handling)

### Maintenance & Cleanup
- New: Linux/WSL Qt/OpenGL system dependencies documented in setup guides (libegl1-mesa, libgl1-mesa-glx, libxcb-*)
- Chore: Reduced play/pause logging to debug level (less console noise)
- Chore: SNR threshold validation updated to reflect optimal value of 2.5 (was 6.5)
- Chore: Removed unnecessary duration override workaround after MP3 VBR fix
- Chore: Updated to torch.amp.autocast API (removed deprecation warning)
- Chore: Removed unused GAP_DETECTED status from SongStatus enum
- Chore: ConfigStub uses MdxConfig defaults as single source of truth
- Chore: Version script skips push when VERSION already committed (both run.bat and run.sh)
- New: WaveformManager deduplicates waveform jobs, stalls standard lane while metadata loads, and ships with dedicated tests
- Chore: Worker queue + LoadUsdxFiles refactor cooperatively yields to instant tasks, tracks metadata batches, and simplifies progress signals
