...
def run_scenario(name: str, wave: np.ndarray, sr: int, truth_ms: float, config: MdxConfig):
    ...
    detected = detect_onset_in_vocal_chunk(vocal_audio=wave, sample_rate=sr, chunk_start_ms=0.0, config=config)
    ...
    visualize.save_waveform_preview(wave, sr, title, truth_ms, detected, str(out_path), rms_overlay=True)
